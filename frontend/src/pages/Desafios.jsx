...
export default function Desafios() {
  ...
  const buscarDesafios = async () => {
    try {
      ...
      console.log('Desafios carregados:', data.length)
      setDesafios(data)
    } catch (error) {
  ...
  const responder = async (opcao) => {
    ...
    try {
      ...
      if (data.correto && !concluidos.includes(desafioAtual._id)) {
        setConcluidos(prev => [...prev, desafioAtual._id])
      }
    } catch (error) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ color: '#c4b5fd', fontSize: 13 }}>👤 {usuario?.nome}</span>
        ...
        }}>⚡ {usuario?.pontuacaoTotal || 0} XP</span>
